...
const create = async (data) => {
	...
	if(idCareer == '' || idCareer == null || idCareer == undefined || idCareer == ' ') 
	...
	if(period.status != 200)
    	return {code : 400, message : `Error al crear el periodo`, error: period.data.message};
...
const getAll = async () => {
	...
	if(period.status != 200)
    	return {code : 400, message : `Error al crear la carrera`, error: period.data.message};
...
const getById = async (id) => {
	...
	if(period.status != 200)
    	return {code : 400, message : `Error al obtener el periodo con el id: ${id}`, error: period.data.message};
