...
const create = async (data) => {
	...
	if(subject.status != 200)
    	return {code : 400, message : `Error al crear la materia`, error: subject.data.message};
...
const getAll = async () => {
	...
	const subject = await getApi("career/all");
...
const getById = async (id) => {
	...
	if(subject.status != 200)
    	return {code : 400, message : `Error al obtener la materia con el id: ${id}`, error: subject.data.message};
